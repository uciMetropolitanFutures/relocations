...
shinyServer(function(input, output) {
  ...
  finalMap <- reactive ({
    ...
    pal <- colorBin("Blues", choice, bins=c(0, 1, 10, 25, 100, 25000), na.color="#FFFFFF")
  ...
  output$within <- renderText({
    paste("Number of relocations WITHIN", input$city, ":", dfsub[,grep(options$choose, colnames(dfsub))][dfsub$NAME10==input$city])
  })
